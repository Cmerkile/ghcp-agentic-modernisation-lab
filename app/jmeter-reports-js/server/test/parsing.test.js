...
import { detectFormat, JtlParseError, parseCsv, parseJtl, parseXml } from '../src/models/parsing/index.js';
...
<httpSample t="120" ts="1700000000000" s="true" lb="Home &amp; Landing" rc="200" by="1024">
...
<httpSample t="300" ts="1700000000500" s="false" lb="Search" rc="500" by="512"/>
</testResults>`;
...
test('parseCsv supports semicolon delimiters and quoted labels', () => {
  const result = parseCsv(
    ['timeStamp;elapsed;label;responseCode;success', '1700000000000;150;"Search; page";200;true'].join(
      '\n',
    ),
  );
...
test('parseCsv skips malformed rows but keeps valid ones', () => {
  const result = parseCsv(
    ['timeStamp,elapsed,label,success', 'not-a-date,abc,Broken,true', '1700000000000,100,Ok,true'].join(
      '\n',
    ),
  );
...
test('parseCsv rejects a header without the required columns', () => {
  assert.throws(
    () => parseCsv('foo,bar\n1,2'),
    (error) => error instanceof JtlParseError && error.code === 'MISSING_COLUMNS',
  );
});
...
test('parseCsv rejects a file with only unusable rows', () => {
  assert.throws(
    ...
    (error) => error instanceof JtlParseError && error.code === 'NO_SAMPLES',
  );
});
...
test('parseXml rejects XML that is not a JMeter result file', () => {
  assert.throws(
    ...
    (error) => error instanceof JtlParseError && error.code === 'INVALID_XML',
  );
});
...
test('parseJtl dispatches on the detected format and rejects empty input', () => {
  ...
  assert.throws(
    () => parseJtl('   \n  '),
    (error) => error instanceof JtlParseError && error.code === 'EMPTY_FILE',
  );
});
